...
export function calculateSettlements(
  ...
  for (const expense of expenses) {
    ...
    for (const participant of expense.participants) {
      ...
      const currentAmount =
        settlementsMap.get(key) ?? 0

      settlementsMap.set(
        key,
        round(currentAmount + share),
      )
    }
  }
  ...
  for (
    const [key, amount]
    of settlementsMap.entries()
  ) {
    const { from, to } =
      parseSettlementKey(key)

    const reverseKey =
      getSettlementKey(to, from)

    if (
      compensatedMap.has(key) ||
      compensatedMap.has(reverseKey)
    ) {
      continue
    }

    const reverseAmount =
      settlementsMap.get(reverseKey) ?? 0

    const netAmount =
      round(amount - reverseAmount)
